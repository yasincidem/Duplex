package com.yasincidem.duplex.common.composable
...
fun OnStart(callback: () -> Unit) {
    OnLifecycle(
        predicate = { it == Lifecycle.Event.ON_START },
        callback = callback
    )
}
...
fun OnCreate(callback: () -> Unit) {
    OnLifecycle(
        predicate = { it == Lifecycle.Event.ON_CREATE },
        callback = callback
    )
}
...
fun OnStop(callback: () -> Unit) {
    OnLifecycle(
        predicate = { it == Lifecycle.Event.ON_STOP },
        callback = callback
    )
}
